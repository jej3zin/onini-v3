...
/* ===== S T A T I C - U S E R S (profiles) ===== */
const users = {
  Yuro: {
    key: 'Yuro',
    name: 'Yuro 🧡',
...
/* =====  L I B R A R Y - S Y S T E M (load items, filter favorites)
...
/* ===== F A V O R I T E S - S Y S T E M (render simples) ===== */
...
/* ===== F E E D - S Y S T E M (profile posts grid) ===== */
...
/* ===== F R I E N D -  S Y S T E M (single implementation: grid + table)
...
// helper: normaliza string (lowercase, trim, sem emojis simples)
function normalizeName(s = '') {
  return (
    s
      ...
      .trim()
      .toLowerCase()
      // remove emojis/basic non-word chars keeping letters/numbers/spaces
...
function visitFriend(id) {
  ...
  // se o friend já tiver uma key/username direto, usa isso (recomendado)
  if (f.key) {
    window.location.href = `/profile/?user=${encodeURIComponent(f.key)}`;
    return;
  }
  if (f.username) {
    window.location.href = `/profile/?user=${encodeURIComponent(f.username)}`;
    return;
  }

  // tenta achar pela lista users (comparando nomes de forma normalizada)
  const target = Object.keys(users).find((k) => {
    const u = users[k];
    if (!u || !u.name) return false;
    return normalizeName(u.name) === normalizeName(f.name);
  });

  if (target) {
    window.location.href = `/profile/?user=${encodeURIComponent(target)}`;
    return;
  }

  // fallback: tenta pela primeira parte do nome (ex: "Ada Wong" -> "ada")
  const first = normalizeName(f.name).split(' ')[0];
  const guess = Object.keys(users).find((k) => normalizeName(k) === first);
  if (guess) {
    window.location.href = `/profile/?user=${encodeURIComponent(guess)}`;
    return;
  }

  // último recurso: notifica (ou você pode abrir uma pesquisa)
  alert('Perfil não encontrado: ' + f.name);
}
